...
$datos=mostrardatosIngreso($myString,$mes);
$num=count($datos);
...
$pdf->SetFont('Arial','B',10);
  ...
  $pdf->Cell(85, 6, 'Monto', 0, 1, 'R', false);
  $pdf->Cell(148, -6, 'Descripcion', 0, 1, 'R', false);
...
for($i=0;$i<$num;$i++){
  ...
  $pdf->Ln(2);
  $pdf->Cell(85, -3, $datos[$i]['nothtax'], 0, 1, 'R', false);
  $pdf->Ln(2);
  $pdf->Cell(148, -2, $datos[$i]['cref'], 0, 1, 'R', false);
  $pdf->Ln(2);
  $pdf->Cell(155, -2, trim($datos[$i]['cplnid']), 0, 1, 'R', false);
  $pdf->Ln(2);
  $pdf->Cell(170, -2, trim($datos[$i]['dcheck']), 0, 1, 'R', false);

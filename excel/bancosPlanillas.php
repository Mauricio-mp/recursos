...
->setCellValue('A3', 'Codigo Empleado')
->setCellValue('B3', 'Apellido')
->setCellValue('C3', 'Nombre')
->setCellValue('D3', 'Identidad')
->setCellValue('E3', 'Monto')
->setCellValue('F3', 'Cuenta')
->setCellValue('G3', 'Descripcion')
->setCellValue('H3', 'Tipo de planilla');
...
$total=$var[$i]['nchkamt']+$total;
    $objPHPExcel->getActiveSheet()->SetCellValue('A'.$cont, $var[$i]['cempno']);
    $objPHPExcel->getActiveSheet()->SetCellValue('B'.$cont, $var[$i]['clname']);
    $objPHPExcel->getActiveSheet()->SetCellValue('C'.$cont, $var[$i]['cfname']);
    $objPHPExcel->getActiveSheet()->SetCellValue('D'.$cont, $var[$i]['cfedid']);
    $objPHPExcel->getActiveSheet()->SetCellValue('E'.$cont, $var[$i]['nchkamt']);
    $objPHPExcel->getActiveSheet()->SetCellValue('F'.$cont, $var[$i]['cbankacct1']);
    $objPHPExcel->getActiveSheet()->SetCellValue('G'.$cont, $var[$i]['cbankname1']);
    $objPHPExcel->getActiveSheet()->SetCellValue('H'.$cont, $var[$i]['cplnid']);
    $cont++;
}
...
$objPHPExcel->getActiveSheet()->SetCellValue('G'.$cont,"Total \t".$bancos[$k]['cbankname1'])->getStyle('G'.$cont)->getFont()->setBold(true);

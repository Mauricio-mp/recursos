...
return $row;
 ...
 function mostrardatos($mes,$opcion,$opcionMes){
  ...
  if($opcion==0){
    ...
    from prmisd a, prempy b where a.cempno=b.cempno and YEAR(tmodrec)='$anio' and MONTH(tmodrec)='$meses' and a.cdedcode IN('932','931','930')order by a.tmodrec");
  
  }else{
    $sql=mssql_query("SELECT  a.cempno,b.clname, b.cfname,b.ctaxstate, cdeptno,a.cpayno,cfedid, a.cdedcode, a.cdesc, a.ndedamt,cplnid, a.tmodrec
    from prmisd a, prempy b where a.cempno=b.cempno and cpayno='$mes' and a.cdedcode IN('932','931','930') order by a.tmodrec");
...
for($i=0;$i<count($productResult);$i++){
  ...
  if($i==0){
    echo "Nombre \t Codigo empleado \t Identidad \t Fecha Generada\t Descripcion \t Fecha de Ingreso\t Monto \t Planilla\t Codigo\t Estado\n";
  }
  $productResult[$i]['tmodrec']=str_replace("/", "-", $productResult[$i]['tmodrec']);
  //$productResult[$i]['ndedamt'] = str_replace(".", ",", $productResult[$i]['ndedamt']);
  echo $productResult[$i]['nombre']."\t".$productResult[$i]['cempno']."\t".$productResult[$i]['cfedid']."\t".$productResult[$i]['tmodrec']."\t".$productResult[$i]['cdesc']."\t".$productResult[$i]['tlstdate']."\t".$productResult[$i]['ndedamt']."\t".$productResult[$i]['cplnid']."\t".$productResult[$i]['cdedcode']."\t".$productResult[$i]['ctaxstate']."\n";
}
